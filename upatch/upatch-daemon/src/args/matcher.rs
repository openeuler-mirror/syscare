...
impl ArgMatcher {
    pub fn get_matched_args() -> ArgMatches<'static> {
        clap_app!(syscare_cli =>
            ...
            (@arg config_file: --config_file +takes_value value_name("CONFIG_FILE") default_value(DEFAULT_SOCKET_FILE) "Path for daemon unix socket")
            (@arg data_dir: --data_dir +takes_value value_name("DATA_DIR") default_value(DEFAULT_CONFIG_FILE) "Daemon data directory")
