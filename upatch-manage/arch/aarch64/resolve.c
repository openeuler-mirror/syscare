...
static unsigned long setup_jmp_table(struct upatch_elf *uelf,
    unsigned long jmp_addr, unsigned long origin_addr)
{
    ...
    unsigned long r_type, unsigned long addr)
{
    ...
    if (upatch_process_mem_read(obj->proc, addr, &jmp_addr, sizeof(jmp_addr))) {
    ...
    unsigned long jmp_addr, unsigned long origin_addr)
{
    ...
    for (unsigned int i = 0; i < uelf->jmp_cur_entry; ++i) {
