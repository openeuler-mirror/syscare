...
#ifndef _ARCH_RISCV64_INSN_H
#define _ARCH_RISCV64_INSN_H

static inline unsigned set_utype_imm(unsigned ins, unsigned long imm)
{
    /* + imm[11] to counteract lo12 sign extension in next instruction */
    unsigned long temp_imm = imm;
    temp_imm += (temp_imm & 0x800) << 1;
    return (temp_imm & 0xfffff000) | (ins & 0xfff);
}

static inline unsigned set_itype_imm(unsigned ins, unsigned long imm)
{
    return ((imm & 0xfff) << 20) | (ins & 0xfffff);
}

static inline unsigned set_stype_imm(unsigned ins, unsigned long imm)
{
    /*                             rs2        rs1   func              opcode
...
static inline unsigned set_jtype_imm(unsigned ins, unsigned long imm)
{
    /*
...
static inline unsigned set_btype_imm(unsigned ins, unsigned long imm)
{
    /*                                rs2        rs1   func                opcode
...
static inline unsigned short set_cjtype_imm(unsigned short ins, unsigned long imm)
{
    /*             funct3       imm                          opcode
...
/* only support C.LUI */
static inline unsigned short set_citype_imm(unsigned short ins, unsigned long imm)
{
...
/* only support C.BEQZ C.BNEZ */
static inline unsigned short set_cbtype_imm(unsigned short ins, unsigned long imm)
{
    /*             funct3  imm[8 4 3]     rs  imm[7 6 2 1 5]      opcode
       ins:         111        0,0 0     11,1     0 0 0,0 0         11
       ins mask         e             3       8                 3

       imm bit no.   8   , 7 6   5   4,3   2 1     0
       imm mask      100   c0    20  18     6

        ==>imm bit no.  12     6-5   2  11-10  4-3
     */
    return (ins & 0xe383) |
           ((imm & 0x100) << (12 - 8)) | ((imm & 0xc0) >> (7 - 6)) |
		   ((imm & 0x20) >> (5 - 2)) | ((imm & 0x18) << (11 - 4)) |
		   ((imm & 0x6) << (4 - 2));
}
