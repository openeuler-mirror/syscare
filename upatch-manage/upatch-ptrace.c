...
int upatch_process_mem_read(struct upatch_process* proc,
                            unsigned long src,
                            void* dst,
                            size_t size)
{
                                           ...
                                           size_t size)
{
    ...
    while (ROUND_DOWN(size, sizeof(long)) != 0) {
        ret =
            ptrace(PTRACE_POKEDATA, proc->pid, dst, *(const unsigned long*)src);
        if (ret) {
            return -1;
        }
        dst += sizeof(long);
...
int upatch_mprotect_remote(struct upatch_ptrace_ctx* pctx,
                           unsigned long addr,
                           size_t length,
                           unsigned long prot)
{
    ...
    ret = upatch_arch_syscall_remote(pctx, __NR_mprotect, (unsigned long)addr,
...
int upatch_munmap_remote(struct upatch_ptrace_ctx* pctx,
                         unsigned long addr,
                         size_t length)
{
    long ret;
