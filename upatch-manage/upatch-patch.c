...
static GElf_Off calculate_load_address(struct running_elf* relf, bool check_code)
{
    int i;
    GElf_Off min_addr = (unsigned long)-1;

    /* TODO: for ET_DYN, consider check PIE */
    if (relf->info.hdr->e_type != ET_EXEC && relf->info.hdr->e_type != ET_DYN) {
    ...
    for (i = 0; i < relf->info.hdr->e_phnum; ++i) {
        ...
        if (!check_code || (check_code && (relf->phdrs[i].p_flags & PF_X))) {
...
static unsigned long calculate_mem_load(struct object_file* obj)
{
    ...
    list_for_each_entry(ovma, &obj->vma, list) {
        if (ovma->inmem.prot & PROT_EXEC) {
            load_addr =
                (load_addr > ovma->inmem.start) ? ovma->inmem.start : load_addr;
        }
    }
...
static int rewrite_section_headers(struct upatch_elf* uelf)
{
    ...
    for (i = 1; i < uelf->info.hdr->e_shnum; i++) {
        ...
        log_debug("section %s at 0x%lx\n", uelf->info.shstrtab + shdr->sh_name,
                  shdr->sh_addr);
    }
...
static void layout_sections(struct upatch_elf* uelf)
{
    enum SectionType {
        ...
        RO_TEXT_AND_RODATA = 1, /* RO: text and ro-data */
        RO_AFTER_INIT = 2,      /* RO after init */
        WHOLE_CORE = 3          /* whole core */
    };
    ...
    for (m = 0; m < ARRAY_SIZE(masks); ++m) {
        ...
        switch (m) {
            case EXECUTABLE:
                ...
                uelf->core_layout.text_size = uelf->core_layout.size;
                break;
            case RO_TEXT_AND_RODATA:
                uelf->core_layout.size = PAGE_ALIGN(uelf->core_layout.size);
                uelf->core_layout.ro_size = uelf->core_layout.size;
                break;
            case RO_AFTER_INIT:
                uelf->core_layout.size = PAGE_ALIGN(uelf->core_layout.size);
                uelf->core_layout.ro_after_init_size = uelf->core_layout.size;
                break;
            case WHOLE_CORE:
                uelf->core_layout.size = PAGE_ALIGN(uelf->core_layout.size);
                break;
...
static void* upatch_alloc(struct object_file* obj, size_t sz)
{
    ...
    addr = upatch_mmap_remote(
        proc2pctx(obj->proc), addr, sz, PROT_READ | PROT_WRITE | PROT_EXEC,
        MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, (unsigned long)-1, 0);
    if (addr == 0) {
        return NULL;
    }
...
static int post_memory(struct upatch_elf* uelf, struct object_file* obj)
{
    ...
    log_debug("Post kbase %lx(%lx) to base %lx\n",
              (unsigned long)uelf->core_layout.kbase, uelf->core_layout.size,
...
static int upatch_info_alloc(struct upatch_elf* uelf,
...
static void upatch_info_init(struct upatch_elf* uelf,
                             struct upatch_info* uinfo)
{
    ...
    uinfo->changed_func_num =
        ufuncs->sh_size / sizeof(struct upatch_patch_func);
                         ...
                         const char* uuid)
{
    ...
    uinfo->end = (unsigned long)uelf->core_layout.base + uelf->core_layout.size;
    uinfo->changed_func_num =
...
#ifdef __riscv
        upatch_func->new_insn = get_new_insn(upatch_func->addr.old_addr,
                                             upatch_func->addr.new_addr);
#else
        upatch_func->new_insn = get_new_insn();
#endif
        log_debug("\t0x%lx(0x%lx 0x%lx -> 0x%lx 0x%lx)\n",
                  upatch_func->addr.old_addr, upatch_func->old_insn[0],
                  upatch_func->old_insn[1], upatch_func->new_insn,
                  upatch_func->addr.new_addr);
    }
                         ...
                         unsigned long changed_func_num)
{
    ...
    for (unsigned int i = 0; i < changed_func_num; ++i) {
        ...
        int ret = upatch_process_mem_write(
            obj->proc, &funcs[i].old_insn,
...
static int apply_patch(struct upatch_elf* uelf, struct object_file* obj)
{
    ...
    for (i = 0; i < uinfo->changed_func_num; ++i) {
        ...
        if (ret) {
            log_error("Failed to ptrace upatch func at 0x%lx(0x%lx) - %d\n",
                      upatch_func->addr.old_addr, upatch_func->new_insn, ret);
            goto out;
        }
        // write 64bit new addr to second 8 bytes
        ret = upatch_process_mem_write(
            obj->proc, &upatch_func->addr.new_addr,
            (unsigned long)upatch_func->addr.old_addr + get_upatch_insn_len(),
            get_upatch_addr_len());
        if (ret) {
            log_error("Failed to ptrace upatch func at 0x%lx(0x%lx) - %d\n",
...
static int upatch_mprotect(struct upatch_elf* uelf, struct object_file* obj)
{
    ...
    if (uelf->core_layout.text_size > 0) {
        ret = upatch_mprotect_remote(
            proc2pctx(obj->proc), (unsigned long)uelf->core_layout.base,
    ...
    if (uelf->core_layout.ro_size > uelf->core_layout.text_size) {
        ret = upatch_mprotect_remote(
            ...
            uelf->core_layout.ro_size - uelf->core_layout.text_size, PROT_READ);
        if (ret < 0) {
    ...
    if (uelf->core_layout.info_size > uelf->core_layout.ro_after_init_size) {
        ret = upatch_mprotect_remote(
            proc2pctx(obj->proc),
            (unsigned long)uelf->core_layout.base +
    ...
    if (uelf->core_layout.size > uelf->core_layout.info_size) {
        ret = upatch_mprotect_remote(
            ...
            uelf->core_layout.size - uelf->core_layout.info_size, PROT_READ);
        if (ret < 0) {
                                ...
                                const char* uuid)
{
    ...
    log_debug(
        ...
        uelf->core_layout.ro_after_init_size, uelf->core_layout.info_size,
        uelf->core_layout.size);
